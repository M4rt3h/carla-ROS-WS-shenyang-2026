...
open_t "NPC Traffic" "t3" "cd \"$CARLA_PATH/PythonAPI/examples\" && PYTHONPATH=\"$CARLA_EGG:\$PYTHONPATH\" python3 generate_traffic.py -n $NPC -w $WEATHER --safe --asynch"
...
open_t "yolo_detector" "t6b" "$ROS_SOURCE && /usr/bin/python3 $WS/install/carla_planning/lib/python3.8/site-packages/carla_planning/yolo_detector_node.py"
wait_bar 2 "Chargement YOLO..."

echo -e "${B}▶ [6c/8]${N} Traffic Light Specialist"
open_t "traffic_light" "t6c" "$ROS_SOURCE && /usr/bin/python3 $WS/install/carla_planning/lib/python3.8/site-packages/carla_planning/traffic_light_node.py"
wait_bar 1 "Chargement spécialiste feux..."

echo -e "${B}▶ [6d/9]${N} Speed Limit Specialist"
open_t "speed_limit" "t6d" "$ROS_SOURCE && /usr/bin/python3 $WS/install/carla_planning/lib/python3.8/site-packages/carla_planning/speed_limit_node.py"
