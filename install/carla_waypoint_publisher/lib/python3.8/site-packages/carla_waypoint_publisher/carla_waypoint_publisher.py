...
class CarlaToRosWaypointConverter(CompatibleNode):
    ...
    - a new goal is set
    """
    WAYPOINT_DISTANCE = 2.0 # 常量用于判断车辆位置变化是否“足够大”以重新规划。
    ...
    def __init__(self):
        ...
        # 初始化两个服务，提供根据位置/actor查找waypoint的能力
        ...
        self.goal_subscriber = self.new_subscription(
            ...
            self.on_goal,
            qos_profile=10)
        # 订阅RViz发布的2D Nav Goal
        # self.goal_subscriber = self.new_subscription(
        #     PoseStamped,
        #     "/goal_pose".format(self.role_name),
        #     self.on_goal,
        #     qos_profile=10)
        
        # 用on_tick回调持续检测ego车辆状态
    ...
    def on_goal(self, goal):
        ...
        :return:
        """
        # 接收到2D Nav Goal后，将其保存并触发重新规划
        self.loginfo("Received goal, trigger rerouting...")
        carla_goal = trans.ros_pose_to_carla_transform(goal.pose)
        self.loginfo("Carla Goal: x=%.3f, y=%.3f, z=%.3f" % (
        carla_goal.location.x, carla_goal.location.y, carla_goal.location.z))
    ...
    def calculate_route(self, goal):
        ...
        """
        # 用Carla官方全局路由规划器计算从ego当前位置到目标点的路线
    ...
    def publish_waypoints(self):
        ...
        """
        # 将所有路点转换成ROS标准PoseStamped，合并为Path并发布
        msg = Path()
        ...
        if self.current_route is not None:
            for i, wp in enumerate(self.current_route):
                ...
                # if i < 5:  # 只打印前5个，避免log爆炸
                #     self.loginfo("  Waypoint %d: x=%.3f, y=%.3f, z=%.3f" % (
    ...
    def connect_to_carla(self):
        # 等待Carla world消息到来，连接成功后保存world对象。
