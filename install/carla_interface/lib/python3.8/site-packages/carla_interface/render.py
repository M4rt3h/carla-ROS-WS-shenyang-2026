...
class BirdeyeRender(object):
  ...
  def render(self, display, render_types=None):
    ...
    if self.hero_actor is not None:
      ...
      clipped_surface = pygame.Surface((self.params["screen_size"]))


      rotated_result_surface = pygame.transform.rotozoom(self.hero_surface, angle, 1.0).convert()

      center = (clipped_surface.get_height() / 2, clipped_surface.get_width() / 2)
      rotation_pivot = rotated_result_surface.get_rect(center=center)
      
      # print(rotation_pivot)
      # clipped_surface.blit(rotated_result_surface, (0, 0), pygame.Rect(clip_x, clip_y, 256, 256))
      clipped_surface.blit(rotated_result_surface, rotation_pivot)
      # display.blit(clipped_surface, (512, 512))

      # print(clipped_surface.get_height())
      # print(clipped_surface.get_width())
