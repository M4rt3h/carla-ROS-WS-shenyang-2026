#include "carla_lqr_control/carla_lqr_pid_control.h"
...
namespace control
{
    ...
    void CarlaLqrPidControlPublisher::declareParameters()
    {
    ...
    void CarlaLqrPidControlPublisher::updateParameters()
    {
    ...
    rcl_interfaces::msg::SetParametersResult CarlaLqrPidControlPublisher::onParameterChange(const std::vector<rclcpp::Parameter> &params)
    {
    ...
    //构造函数，初始化
    CarlaLqrPidControlPublisher::CarlaLqrPidControlPublisher(): Node("carla_stanley_pid_control_publisher")
    {
        ...
        this->param_callback_handle_ = this->add_on_set_parameters_callback(
            std::bind(&CarlaLqrPidControlPublisher::onParameterChange, this, std::placeholders::_1)
        );

        //订阅车辆位姿
        this->sub_ego_vehicle_odom_ = this->create_subscription<nav_msgs::msg::Odometry>("/carla/hero/odometry", 1, std::bind(&CarlaLqrPidControlPublisher::odomCallback, this, _1));
        //订阅规划路径点
        this->sub_route_waypoint_ = this->create_subscription<nav_msgs::msg::Path>("/carla/hero/waypoints", 1, std::bind(&CarlaLqrPidControlPublisher::routeWaypointCallback, this, _1));
        //发布车辆控制信息
        ...
        std::chrono::milliseconds(50), std::bind(&CarlaLqrPidControlPublisher::controlIterationCallback, this));

        //纵向和横向的pid控制器
        this->lon_pid_ = std::make_unique<control::PID>(0.05, 1.0, -1.0, LonPID_Kp_, LonPID_Ki_, LonPID_Kd_);
        this->lat_controller = std::make_unique<control::LqrController>();

    }

    void CarlaLqrPidControlPublisher::odomCallback(const nav_msgs::msg::Odometry::SharedPtr msg)
    {
        //获取当前ego-vehicle的位姿
        // 姿态角获取
    ...
    void CarlaLqrPidControlPublisher::routeWaypointCallback(const nav_msgs::msg::Path::SharedPtr msg)
    {
    ...
    void CarlaLqrPidControlPublisher::updateTrajectorySpeed()
    {
    ...
    void CarlaLqrPidControlPublisher::controlIterationCallback()
    {
        if (plan_waypoints_.empty()) return;
        if (SPEED_UPDATED)
        {
        ...
        double steer_cmd = this->lat_controller->ComputeControlCommand(curr_vehicle_state_, target_point_);
    ...
    double CarlaLqrPidControlPublisher::PointDistanceSquare(const TrajectoryPoint &point, const double x, const double y)
    {
        double dx = point.x - x;
        double dy = point.y - y;
        return dx * dx + dy * dy;
    ...
    TrajectoryPoint CarlaLqrPidControlPublisher::QueryNearestPointByPosition(const double x, const double y)
    {
...
int main(int argc, char const* argv[])
{
    ...
    rclcpp::spin(std::make_shared<control::CarlaLqrPidControlPublisher>());
    rclcpp::shutdown();
    return 0;
}