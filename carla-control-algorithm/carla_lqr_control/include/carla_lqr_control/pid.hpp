...
namespace control
{
  ...
  public:
    PID() {}; //构造函数
    PID(double dt, double max, double min, double Kp, double Kd, double Ki);  //构造函数
    ~PID() {};  //析构函数

    void updateSettings(const double Kp, const double Kd, const double Ki);  // 函数接口
    // Returns the manipulated variable given a setpoint and current process value
    double calculate(const double error);  //函数接口

  private:
    double dt_; //控制周期（时间间隔）
    double max_; //输出上限
    double min_; //输出下限
    double Kp_; //比例增益
    double Kd_; //微分增益
    double Ki_; //积分增益
    double pre_error_; //上一次误差值（用于计算微分项）
    double integral_; //积分累积值（历史误差总和）
    // 改进部分
    // 使用队列存储最近的误差
    std::queue<double> error_queue_;
    size_t max_queue_size;  // 仅保存最近20个误差
  };
